...
最終更新: 2026-09-22（リョウによる再採点：85/100。前回2026-08-19時点86〜88から横ばい。
運用・セキュリティ面（死活監視完成・RLS強化・dependabot追加）は伸びたが、買い切りプラン検討で
事業要件側に新しい未確認事項（規約はterms.mdで先行改訂したがbilling/plans.tsは未対応、DPA未整備）
が増えたため相殺。詳細は次回会話ログ参照）
...
## D. スコアを上げる（90→その先）
- 🤖 完了（2026-09-22確認）：UX整理。`AdminNav.tsx`を確認したところ、9タブが
  `main/work/settle/manage`の4グループに整理され、区切り線・アクティブタブの
  自動スクロールも実装済みだった（本項目を書いた時点からいつの間にか対応されていた）。
  さらなる刷新（サイドバー化・ドロップダウン化等）は見た目の方針変更になるため、
  やるなら次はオーナー判断で
- 🧑 実顧客の獲得（“解約ゼロ実績”は時間でしか買えない加点）

## G. 最小運用体制まわり（2026-09-22・買い切りプラン検討に伴う洗い出し）
- 🤖 完了：`.github/dependabot.yml` 追加（npm・github-actions の週次更新チェック。脆弱性検知の自動化）
- 🤖 完了：`terms.md` 第12条3項に「月額プラン／買い切りプラン」で賠償上限の算定基準を分岐する改訂を反映
  （未レビュー。`弁護士相談資料.md` 論点2に追記済み）
- 🧑 未実施：リストア訓練（RUNBOOK 3章）。復元テスト用に作った旧Supabaseプロジェクトの扱い
  （削除 or staging化）も上の「未決定」項目と合わせて決める
- 🧑 未実施：`Database → Backups` に実際に日次バックアップが並んでいるかの確認
- 🧑 未検討：賠償責任保険（サイバー保険・IT賠償責任保険）への加入。故意・重過失時は
  賠償上限が外れる設計のため、支払い能力の裏付けとして検討推奨
- 🧑 最優先：下記D2（社労士・弁護士への実相談）が終わらないと、上記の免責設計は
  「一次レビュー止まり」のまま。買い切りプランを実売する前に必ず送付する
  （2026-09-22：知り合いに依頼する可能性もあるとのことで、一旦保留中）
- 🧑 未着手・要方針決定：`terms.md`は買い切りプランの賠償上限条項を先行改訂したが、
  肝心の`src/lib/billing/plans.ts`（`PlanId`型・`Subscription`）にはまだ「買い切り」という
  区分自体が存在しない。実装するには、一括支払いの金額・スタッフ数上限・オプション課金・
  保守契約の扱いをオーナーが決める必要がある（設計が決まればカイが実装可能）
