...
> ## ⚠️ 2026-09-09 確認：現在 Free Plan のため自動バックアップが一切無い
> Supabase → Database → Backups で確認したところ、
> **「Free Plan does not include project backups.」**と表示されている。
> つまり**今、本番で事故（テーブルの誤削除・データ破損等）が起きても、復元する手段が無い。**
> 3章の「リストア」手順は、バックアップが存在することを前提にしており、**現状は実行できない**。
> Pro Planへのアップグレード（有料。コスト発生のためオーナー判断＝rules.md Rule 1）が
> 必要かどうか、早めに検討すること。
...
- **手動バックアップ（重要変更の前に推奨）**:
  ...
  - ⚠️ Free Planでは`Database → Backups`の手動バックアップ機能自体が使えない可能性が高い
    （2026-09-09時点でボタンが有料プラン誘導のみだった）。**`pg_dump`によるローカル保存が、
    Free Planのままなら唯一の現実的なバックアップ手段**。
- **マイグレーション前**は必ずバックアップ（rules.md Rule 1 準拠）。additive設計だが保険。
  Free Planの間は、上記の`pg_dump`を必ず実施すること（ダッシュボードのボタンに頼らない）。

## 3. リストア（復旧）

> ⚠️ **2026-09-09時点、この手順は前提が崩れている**：本番がFree PlanのためSupabase側の
> バックアップが無い（2章参照）。Pro Planへのアップグレード、または`pg_dump`でのローカル
> バックアップ運用を始めるまで、以下の「Backupsから復元」は実質的に使えない。
