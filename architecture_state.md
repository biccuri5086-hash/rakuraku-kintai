...
- `らくらく勤怠/specs/弁護士相談資料.md`：スポット相談用の論点整理・依頼先の見つけ方・打診メール。
  **2026-09-09時点、まだ実際には送付していない（依頼先も未確定）**。
- `らくらく勤怠/specs/労務レビュー_ミオ一次チェック.md`：給与・派遣法の一次レビュー詳細。
  高リスク2件（月60h超割増・クーリング期間）は実装済みを確認済み。
- `らくらく勤怠/specs/社労士相談資料.md`：上記を踏まえたスポット相談資料・依頼先の見つけ方・打診メール。
  **2026-09-09時点、まだ実際には送付していない**（`sales/12_社労士短文打診メール.md`は
  紹介パートナー契約の営業メールで別目的なので混同しないこと）。
